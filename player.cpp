...
void player::playStep(){
    if(playingStatus){
        ...
        if(round(tp-tempo)!=0){
            tempo=tp;
            //::__android_log_print(ANDROID_LOG_INFO, "mgenner","set tempo:%f\n",tempo);
        }
        ...
        for(auto it:toPlay){
            ...
            if(ins>=0){
                ...
                //::__android_log_print(ANDROID_LOG_INFO, "mgenner","fail to play note:%s(%d)\n",it->info.c_str(),(int)it->tone);
            }
        }
        toPlay.clear();
...
int player::useInstrument(const stringPool::stringPtr & n){
...
int player::releaseInstrument(const stringPool::stringPtr & n){
    int id = getInstrumentId(n);
    if(ins2Channel[id]==-1){
        return -1;
    }else{
...
void player::noteOff(note * n){
    if(n){
        ...
        if(ins>=0){
            ...
            //::__android_log_print(ANDROID_LOG_INFO, "mgenner","fail to release note:%s(%d)\n",n->info.c_str(),(int)n->tone);
        }
    }
}
