...
bool midiMap::addTempo(int tick,double tp){
    if(timeMap.find(tick)==timeMap.end()){
        //::__android_log_print(ANDROID_LOG_INFO, "mgenner","add tempo:%d %f\n",tick,tp);
        timeMap[tick]=tp;
        return true;
    }else{
        return false;
    ...
    if(!info.empty() && info[0]=='@' && info.size()>2){
        ...
        if(str[1]=='T'){
            ...
            if(strlen(r)>0){
                int tick=atoi(r);
                if(tick>0){
                    //::__android_log_print(ANDROID_LOG_INFO, "mgenner","remove tempo:%f %d\n",begin,tick);
                    timeMap.erase(begin);
                }
            }
        }
    ...
    if(!info.empty() && info[0]=='@' && info.size()>2){
        ...
        if(str[1]=='T'){
            ...
            if(strlen(r)>0){
                double tick=atof(r);
                if(tick>0){
                    //::__android_log_print(ANDROID_LOG_INFO, "mgenner","add tempo:%f %f\n",begin,tick);
                    timeMap[begin]=tick;
                }
            }
        }else
        if(str[1]=='S'){
            ...
            if(strlen(r)>0){
                int sec=atoi(r);
                if(sec>0 && sec<8){
                    //::__android_log_print(ANDROID_LOG_INFO, "mgenner","set section:%d\n",sec);
                    setSection(sec);
                }
            }
        }
